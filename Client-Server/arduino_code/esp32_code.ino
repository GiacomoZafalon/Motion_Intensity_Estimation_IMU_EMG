...
const int port = 3331; // Port number on which your server is listening (Sensor1-3331, Sensor2-3332, Sensor3-3333, Sensor4-3334)
...
void loop() {
  ...
  sendSensorData(euler.x(), euler.y(), euler.z(), acc.acceleration.x, acc.acceleration.y, acc.acceleration.z, gyr.gyro.x, gyr.gyro.y, gyr.gyro.z, mag.magnetic.x, mag.magnetic.y, mag.magnetic.z, linAcc.acceleration.x, linAcc.acceleration.y, linAcc.acceleration.z);

  delay(BNO055_DELAY_MS); // Adjust delay as needed
}

void sendSensorData(float euler_x, float euler_y, float euler_z, float accel_x, float accel_y, float accel_z, float gyro_x, float gyro_y, float gyro_z, float mag_x, float mag_y, float mag_z, float linear_accel_x, float linear_accel_y, float linear_accel_z) {
