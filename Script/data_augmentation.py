...
def process_file(file_path, output_dir, warp_type=2, amount_of_warping=10, std_dev=0.01):
    ...
    augmented_data.to_csv(output_dir, index=False, header=None)

def augment_data(person, weight, attempt, time_step, tot_times, base_dir, tot_person, tot_weight, std_dev=0.1):
    ...
    augmented_imu_data_dir = os.path.join(base_dir, f'P{person}/W{weight}/A{attempt + time_step}/imu/data_neural_euler_acc_gyro.csv')
    augmented_emg_data_dir = os.path.join(base_dir, f'P{person}/W{weight}/A{attempt + time_step}/emg/emg_label.csv')
    ...
    for weight in range(1, tot_weights + 1):
        for attempt in range(1, tot_attempts + 1):
            for i in range(1, times + 1):
                augment_data(person, weight, attempt, i, times, base_dir, tot_persons, tot_weights)
